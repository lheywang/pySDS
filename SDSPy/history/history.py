...
class SiglentHistory(SiglentBase):
    ...
    def EnableHistory(self):
        ...
        pySDS [History][EnableHistory] : Enable the History mode
            ...
            Returns :
                True | False : Has been enabled or not
        """
        ret = self.__instr__.query("HSMD ON").strip().split(" ")[-1]
        if ret == "ON":
            return True
        return False
    ...
    def DisableHistory(self):
        ...
        pySDS [History][DisableHistory] : Disable the History mode
            ...
            Returns :
                True | False : Has been disabled or not
        """
        ret = self.__instr__.query("HSMD OFF").strip().split(" ")[-1]
        if ret == "OFF":
            return True
        return False
    ...
    def GetFrameAcquisitionTime(self):
        ...
        if ret.split(" ")[0] == "FTIM":
            ...
            return datetime.time(hour=ret[0].strip(), minute=ret[1].strip(), second=ret[2].strip(), microsecond=ret[3].strip())
        
        else:
            # Get measurement parameters
            samples = int(ret.replace("\x", ""), 16)
    ...
    def EnableHistoryList(self):
        ...
        pySDS [History][EnableHistoryList] : Enable the list mode
            ...
            Returns :
                True | False : Has been enabled or not
        """
        ret = self.__instr__.query("HSLST ON").strip().split(" ")[-1]
        if ret == "ON":
            return True
        return False
    ...
    def DisableHistoryList(self):
        ...
        pySDS [History][DisableHistoryList] : Disable the list mode
            ...
            Returns :
                True | False : Has been disabled or not
        """
        ret = self.__instr__.query("HSLST OFF").strip().split(" ")[-1]
        if ret == "OFF":
            return True
        return False
